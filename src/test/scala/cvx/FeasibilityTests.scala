...
object FeasibilityTests {
  ...
  /** Perform a feasibility analysis (first simple, then SOI) on the
    ...
    * eqs.
    * @param doSOI: carry out a sum of infeasibilities analysis also.
    */
  def checkFeasibility(
    doSOI:Boolean, cts:ConstraintSet,eqs:Option[EqualityConstraint],
  ...
  def checkFeasibilityProbabilitySimplex(
    doSOI:Boolean, n:Int, pars:SolverParams, debugLevel:Int
  ):Unit = {
    ...
    val cts = ConstraintSet(n,cnts,setWhereDefined,x0)
    val probCt = Constraints.sumToOne(n)

    checkFeasibility(doSOI,cts,Some(probCt),pars,debugLevel)
  }
  ...
  def checkRandomFeasibleConstraints(
    doSOI:Boolean,x0:DenseVector[Double],p:Int,q:Int,pars:SolverParams,debugLevel:Int
  ):Unit = {
  ...
  /** Run all the tests with specific parameters.
    * @param doSOI: carry out a sum of infeasibilities analysis also.
    */
   def runAll(doSOI:Boolean,pars:SolverParams, debugLevel:Int):Unit = {
     ...
     checkFeasibility_PAB(doSOI,n,I_A,pA,sgnA,I_B,pB,sgnB,pars,debugLevel)

     val x0 = DenseVector.tabulate[Double](20)(j => 1.0)     // feasible point
     val p=10; val q=5;                                       // number of linear and quadratic inequalities

     for(tests <- 1 to 5)
       checkRandomFeasibleConstraints(doSOI,x0,p,q,pars,debugLevel)
   }
}
