...
object UnconstrainedMinimizationTests {
    ...
    def testList(testFunctions:List[TestFunction],maxIter:Int):Unit = {
        ...
        for(testFunction <- testFunctions) try {
            ...
            val solver = testFunction.solver(x0,C)

            val sol = solver.solve(maxIter,alpha,beta,tol,delta)
            val x = sol._1                    // solution
            val y = testFunction._objF(x)
            val y_opt = testFunction.globalMin
            val newtonDecrement = sol._2      // Newton decrement at solution
            val normGrad = sol._3            // norm of gradient at solution
            val iter = sol._4
            val maxedOut = sol._5
